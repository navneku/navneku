...
# DBTITLE 1,Introduction to this notebook
# MAGIC %md
...
# DBTITLE 1,Make table/yyyyMMdd based directories
# MAGIC %md
...
AdvancedStats_notebook ='03.2_Hive_vs_Synapse_Advanced_Statistics'
...
with open("/dbfs"+dbfs_mount_path + wandisco_container_name + dest_path + AdvancedStatsConfig,"r") as read_obj:
    ...
    if header !=None:
        for row in csv_reader:
            if row[6]== "1":
                #New path created: /mnt/wandisco/Test_Validation/AdvancedStats/<schema.tableName>/yyyyMMdd/
                dbutils.fs.mkdirs (dbfs_mount_path + wandisco_container_name + dest_path+row[1]+'.'+row[2]+"/"+DateFolder)
