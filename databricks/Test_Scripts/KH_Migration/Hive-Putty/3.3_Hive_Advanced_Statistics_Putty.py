...
for db_nm,tbl,primary_key in zip(db_nm_list,tbl_list,primary_key_list):
    ...
    hive_primary_key_df.replace('','null').to_csv('{0}Hive_{1}_metadata_count.csv'.format(export_path,tbl),index=False,sep='|')
    dm_cat.replace('','null').to_csv('{0}Hive_{1}_cat.csv'.format(export_path,tbl),index=False,sep='|')
    dm_num.replace('','null').to_csv('{0}Hive_{1}_num.csv'.format(export_path,tbl),index=False,sep='|')
    dm_date.replace('','null').to_csv('{0}Hive_{1}_date.csv'.format(export_path,tbl),index=False,sep='|')
