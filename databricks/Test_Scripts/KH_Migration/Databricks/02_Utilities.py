...
# DBTITLE 1,Percentage Metrics And DateDiff
def value_diff(col1,col2):
    return (col1.astype(float)-col2.astype(float))
def percentage_change(col1,col2):
    return (col1.astype(float)/ col2.astype(float)) * 100
def date_compare(col1,col2):
...
rddArr = []
def get_file_path(path,container):
    #accum_size = 0
    ...
    if path_list:
        for path_object in path_list:
            ...
            if path_object.path[-1] != '/':
                ...
                if(path_object.path[-1] == '/'):
                    get_file_path(path_object.path,container)
...
def oracle_row_column_count(tbl_list):
    ddlSchema = StructType([
                StructField('tblnm',StringType()),
                StructField('columncount',StringType()),
                StructField('rowcount',IntegerType())])
