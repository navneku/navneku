...
#/home/derek mk tsui/Hive_test_script_prod/Hive_schema_tablename_metadata_count.csv
#/home/derek mk tsui/Hive_test_script_prod/Hive_schema_tablename_cat.csv
#/home/derek mk tsui/Hive_test_script_prod/Hive_schema_tablename_num.csv
#/home/derek mk tsui/Hive_test_script_prod/Hive_schema_tablename_date.csv
...
def check_primary_key(prim_key, db_nm, tbl_nm, spark):
    df = hiveWH.executeQuery("select * from {}.{}".format(db_nm,tbl_nm))
                ...
                for metric in date_components:
                    ...
                    df_tmp = spark.sql("select '" + db_nm + "' as schema_name ,'" + tbl_nm
                             + "' as table_name, '" + field + "' as column_name, '" + metric + "' as date_type,cast( " + metric + "(" + field
                                                             + ") as string)as column_value, cast(count(*) as string )as freq from " + tbl_nm
                ...
                for metric in measures:
                    ...
                    df_tmp = spark.sql("select '" + db_nm + "' as schema_name ,'" + tbl_nm
                             + "' as table_name, '" + field + "' as column_name, " + "'boundary' as date_type, '" + metric + "imum' as column_value, cast("
                                                            + metric + "(" + field + ") as string)as freq from " + tbl_nm)#made chngs cast
                    df_tmp = df_tmp.toPandas()
...
tbl_list=[]
primary_key_list=[]
with open('AdvancedStats_Config.csv', "r") as csvfile:
...
#     hive_primary_key_df.replace('','null').to_csv('{}Hive_{}_metadata_count.csv'.format(export_path,tbl),index=False,sep='|')
#     dm_cat.replace('','null').to_csv('{}Hive_{}_cat.csv'.format(export_path,tbl),index=False,sep='|')
#     dm_num.replace('','null').to_csv('{}Hive_{}_num.csv'.format(export_path,tbl),index=False,sep='|')
#     dm_date.replace('','null').to_csv('{}Hive_{}_date.csv'.format(export_path,tbl),index=False,sep='|')
