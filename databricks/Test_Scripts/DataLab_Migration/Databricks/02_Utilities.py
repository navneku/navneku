...
def load_adls_df(file_path,file_format,file_name=None,schema=None):
    ...
    elif file_format=="parquet" or file_format=="orc": #file_format=="json"
        df = spark.read.format(file_format).option("recursiveFileLookup","True").load(path)
...
def percentage_change(col1,col2):
    return ((col1.astype(float)-col2.astype(float))/ col1.astype(float)) * 100

def percentage_changes(col1,col2):
